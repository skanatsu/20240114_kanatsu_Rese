...
- 一般ユーザーとしてログイン済みの場合、ページ上部に並び替えボタンが表示されます。
  - 並び替えボタンを押下すると、並び替え方法を選べる欄が表示されます。
    ...
    - 評価が低い順：店舗の評価が低い順に並び替えられます。
      - 店舗の評価は、各評価の5段階評価の平均値で計算されます。
      - 評価がされていない店舗は、「評価が高い順」「評価が低い順」にかかわらず、最後尾に表示されます。
...
### 評価機能
- 飲食店詳細ページの「口コミを投稿する」リンクを押下すると、その店舗の評価を送信できるページに遷移します。
![](public/images/review1.png)
- 星マークにマウスオーバーすると、色が灰色から青色に変わります。押下することで、満足度に応じて5段階評価を選択できます。
...
- 「画像の追加」欄をクリック、またはドラッグ&ドロップすることで、写真を追加することができます。
  ...
  - アップロード後、「画像を削除」ボタンを押下すると、アップロードした画像を削除できます。

![](public/images/review_image.png)
- 「口コミを投稿」ボタンを押下すると、評価が送信され、当該飲食店の店舗詳細ページに戻ります。「全ての口コミ情報」欄には、送信した評価が表示されます。
  - 評価送信済みの場合、「口コミを編集」ボタン、または「口コミを投稿する」ボタンを押下すると、評価送信ページに遷移します。なお「口コミを削除」ボタンを押下すると、当該評価を削除できます。
    - 内容を更新し、「口コミを投稿」ボタンを押下することで、評価を再送信できます。
...
- ユーザータイプによる仕様の違い
  ...
  - 店舗ユーザー：評価の閲覧は可能ですが、送信・編集・削除はできません。
  - 管理者ユーザー：全ての評価の削除が可能ですが、送信・編集はできません。
...
- ボタンの押下、またはドラッグ&ドロップにより、店舗情報を記載したCSVファイルをアップロードすると、「CSVをインポート」ボタンが表示されます。このボタンを押下すると、店舗情報が追加されます（既存店舗の更新ではなく、新規店舗の追加です）。
![](public/images/csv2.png)

- ファイルの形式や入力の不足、誤りがあると（下記条件のいずれかでも満たしていないと）、エラーメッセージが表示されます。
  - ファイル形式はCSV
  - 項目は全て入力必須
  - 店舗名：50文字以内
