                    ...
                    <div class="reservation__card">
                        ...
                        @if (!$isReservationPast)
                            <div class="qr-code">
                                ...
                                <img src="data:image/png;base64, {!! base64_encode(
                                    QrCode::format('png')->size(200)->generate($reservation->id),
                                ) !!}" class="qr-code__img"
                                    alt="QR Code">
                            </div>
                        @endif
                        @if (!$isReservationPast)
                            <div class="reservation__update">
                                ...
                                <form action="{{ route('reservation.update', ['id' => $reservation->id]) }}"
                                    method="post" class="reservation__update-form">
                                    @csrf
                                    @method('put')
                                    <td colspan="2">
                                        <input type="date" id="date" class="reservation__update__date"
                                            name="date" value="{{ $reservation->date }}" onchange="updateTable()">
                                        ...
                                        <select id="people" name="people" class="reservation__update__number"
                                            onchange="updateTable()">
                                            @for ($i = 1; $i <= 10; $i++)
                                                <option value="{{ $i }}"
                                                    {{ $i == $reservation->number ? 'selected' : '' }}>
                                                    {{ $i }}人</option>
                                            @endfor
                                        </select>
                        ...
                        @if (!$isReservationPast)
                            <div class="reservation__pay">
                                <a href="{{ route('reservation.pay', ['id' => $reservation->id]) }}"
                                    class="reservation__pay__link">来店前の決済はこちら</a>
                            </div>
                        @endif
                    </div>
                @endforeach
                    ...
                    <div class="shop">
                        ...
                        @auth
                            <img src="{{ url('/images/' . ($favorite->shop ? ($favorite->shop->isFavorite ? 'heart.png' : 'greyheart.png') : 'default.png')) }}"
                                alt="" class="heart"
                                data-shop-id="{{ $favorite->shop ? $favorite->shop->id : 0 }}">
                        @endauth
                    </div>
                @endforeach
            @endif
